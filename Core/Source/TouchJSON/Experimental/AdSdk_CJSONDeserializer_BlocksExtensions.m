...
#import "AdSdk_CJSONDeserializer_BlocksExtensions.h"

#import "AdSdk_CJSONScanner.h"

@implementation AdSdk_CJSONDeserializer (CJSONDeserializer_BlocksExtensions)
...
- (void)deserializeAsDictionary:(NSData *)inData completionBlock:(void (^)(id result, NSError *error))block {
	...
	if (inData == NULL || [inData length] == 0) {
		noDataError = [NSError errorWithDomain:AdSdk_kJSONDeserializerErrorDomain code:kJSONScannerErrorCode_NothingToScan userInfo:NULL];
...
- (void)deserializeAsArray:(NSData *)inData completionBlock:(void (^)(id result, NSError *error))block {
	...
	if (inData == NULL || [inData length] == 0) {
		nullInDataError = [NSError errorWithDomain:AdSdk_kJSONDeserializerErrorDomain code:kJSONScannerErrorCode_NothingToScan userInfo:NULL];
